...
@Tag(name = "메뉴(상품) API", description = "메뉴(상품) 목록·상세 조회 API")
@RestController
...
public class ProductController {
    ...
    @Operation(
        summary = "상품 목록 조회",
        description = "페이지네이션, 상태/가격 필터, 검색어, 정렬(최신순/가격순/이름순)을 지원합니다."
    )
    ...
    public ResponseEntity<ApiResponse<ProductListResponse>> getProductList(
        ...
        @Parameter(description = "검색 키워드 (상품명/설명 검색)", example = "노트")
    ...
    @Operation(
        summary = "상품 상세 조회",
        description = "하나의 상품에 대한 상세 정보(재고, 상태, 설명 등)를 조회합니다."
    )
    @io.swagger.v3.oas.annotations.responses.ApiResponses({
